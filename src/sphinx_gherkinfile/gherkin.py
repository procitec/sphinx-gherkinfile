...
from sphinx_gherkinfile import SphinxGherkinFileError, keyword_to_objtype
...
def _no_op_matcher[K: "Keyword"](keyword: K, document: Document) -> bool:
    return True
...
class Document:
    ...
    ancestry: dict[GherkinElement, GherkinElement | None] = field(default_factory=dict)
    ...
    def get_parent_keyword(self, element: GherkinElement) -> Keyword:
        ...
        if isinstance(parent, Keyword):
            ...
            raise SphinxGherkinFileError(f"Ancestry is wrong for {element}.")
    ...
    def find(
        self,
        cls: type[K],
        matcher: Callable[[K, Document], bool] | None = None,
    ) -> Iterator[K]:
        if not matcher:
...
class DefinitionBuildah:
    ...
    def from_path(cls: type[B], filepath: Path) -> B:
        return cls(str(filepath), filepath.read_text(encoding="utf-8"))

    def parse(self, gherkin_parser: Parser | None = None) -> Document:
    ...
    def visit_feature(
        ...
        location = self.visit_location(raw_definition["location"], farthest_code_location)

        if tags:
            first_tag = tags[0]
            location = CodeSpan(start=first_tag.location.start, end=location.end)
    ...
    def visit_children(
        ...
        for raw_child in children_definitions:
            ...
            farthest_code_location = self.find_closer_farthest_location(child.location.start)
    ...
    def visit_scenario(
        ...
        steps = self.visit_steps(raw_definition["steps"], farthest_code_location)
    ...
    def visit_steps(
        ...
        for raw_step in sorted(
            ...
            farthest_code_location = self.find_closer_farthest_location(step.location.start)
            steps.append(step)
    ...
    def visit_step(
        ...
        docstring = self.maybe_visit_docstring(raw_definition, farthest_code_location)

        datatable = self.maybe_visit_datatable(raw_definition, farthest_code_location)
    ...
    def maybe_visit_datatable(
        ...
        try:
            ...
            return None
        return DataTable(tuple(tuple(cell["value"] for cell in row["cells"]) for row in datatable_definition["rows"]))
    ...
    def visit_examples(
        ...
        for raw_examples in reversed(raw_definition):
            one_examples = self.visit_one_examples(raw_examples, farthest_code_location)
            farthest_code_location = self.find_closer_farthest_location(one_examples.location.start)
    ...
    def visit_one_examples(
        ...
        first_row = [cell["value"] for cell in raw_definition["tableHeader"]["cells"]]
        other_rows = [[cell["value"] for cell in row["cells"]] for row in raw_definition["tableBody"]]
        ...
        examples = Examples(
            ...
            tags=tags,
            location=location,
            datatable=DataTable(tuple(tuple(row) for row in rows)),
        )
    ...
    def visit_comments(
        ...
        for raw_comment in raw_definition:
            ...
            comment = Comment(
                ...
                location=CodeSpan(
                    start=start,
                    end=CodeLocation(line=start.line, column=len(self.lines[start.line])),
                ),
            )
